...
export const getEvents = async (): Promise<Event[]> => {
  ...
  return eventsData.default as Event[];
};
...
export const getEventById = async (id: string): Promise<Event | null> => {
  await delay(600);
  const eventsData = await import('../data/events.json');
  const events = eventsData.default as Event[];
 ...
 */
export const getEventsByCompanyId = async (companyId: string): Promise<Event[]> => {
  await delay(700);
  const eventsData = await import('../data/events.json');
  const events = eventsData.default as Event[];
 ...
 */
export const getEventsByStudentId = async (studentId: string): Promise<Event[]> => {
  await delay(700);
  const eventsData = await import('../data/events.json');
  const events = eventsData.default as Event[];
